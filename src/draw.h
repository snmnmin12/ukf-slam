...
public:
 ...
 void Plot_state(const VectorXd& mu, const MatrixXd& sigma, const Mapper& mapper, const vector<int>& observedLandmarks, const vector<RadarReading>& Z) {
	...
	for(int i= 0; i < Z.size(); i++) {
           int index = 0;
	   for (; index < observedLandmarks.size();index++) {
	     if (observedLandmarks[index] == Z[i].id) break;	
	   }
	   vector<float> X, Y;
	   X.push_back(mu(0));Y.push_back(mu(1));
	   X.push_back(mu(2 * index + 3));
	   Y.push_back(mu(2 * index + 4));
	   plt::plot(X, Y, "k");
	}   
